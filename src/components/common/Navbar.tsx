...
const Navbar: React.FC = () => {
  ...
  const navItems = [
    ...
    { to: '/mypage/record/all', label: '기록', icon: <Record className="w-[90%]" /> },
    { to: '/mypage', label: '내 정보', icon: <User className="w-[90%]" stroke="#333333" /> },
  ];
        ...
        >
          {item.to ? (
            <Link to={item.to} className="flex flex-col items-center">
              {item.icon}
              <span className="text-[9px] text-gray-600 -mt-1">{item.label}</span>
            </Link>
          ) : (
